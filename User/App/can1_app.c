...
motor_msg_t thumbwheel_motor_msg = {0};
/**
  * @brief          
  * @author         
  * @param[in] 
...
static void chassis_motor_msg_process(motor_msg_t *m, uint8_t aData[])
{
	...
	m->encoder.last_raw_value = m->encoder.raw_value; 	///数据暂存
...
void can1_message_progress(CAN_RxHeaderTypeDef *pHeader, uint8_t aData[])
{
	...
	switch(pHeader->StdId)
	{
		...
		case CAN2_THUMBWHEEL_MOTOR_STD_ID:
		{
			chassis_motor_msg_process(&thumbwheel_motor_msg ,aData);            
		}break;
	}
}
/**
...
{
	can1_tx_header.StdId = CAN2_THUMBWHEEL_CTRL_STD_ID;
    can1_tx_header.IDE = CAN_ID_STD;
    can1_tx_header.RTR = CAN_RTR_DATA;
    can1_tx_header.DLC = 0x08;
